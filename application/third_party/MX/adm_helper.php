...
if(!function_exists('total_expert_cat')){
	function total_expert_cat($catid){
   	      $ci = ci_instance();
   return $data = $ci->Admin_model->expert_data_row_cat($catid);
	}
}
...
if(!function_exists('subcat_name_subcatwise')){
	function subcat_name_subcatwise($subcatid){
   	      $ci = ci_instance();
   	      $data = array();
      $data = $ci->Admin_model->single_subcat_data($subcatid);
      if($data != false){
          return $data;
      }
	}
...
function cat_name_withsubcat($id,$subcatid){
    $ci = ci_instance();
    return $ci->Admin_model->single_cat_data($id,$subcatid);
}
function subcat_single($subcatid){
    $ci = ci_instance();
    return $ci->Admin_model->subcat_data_single($subcatid);
}
