...
class Expert_api extends REST_Controller{
  ...
  public function __construct(){
    ...
    //load database
    $this->load->database();
    $this->load->model(array("expert_api_model"));
  ...
  */
  ...
  public function index_post(){
    ...
    if($this->form_validation->run() === FALSE){
      ...
      if(!empty($name) && !empty($email) && !empty($mobile) && !empty($course)){
        ...
        if($this->expert_api_model->insert_student($student)){
  ...
  public function index_put(){
    ...
    if(isset($data->id) && isset($data->name) && isset($data->email) && isset($data->mobile) && isset($data->course)){
      ...
      if($this->expert_api_model->update_student_information($student_id, $student_info)){
  ...
  public function index_delete(){
    ...
    if($this->expert_api_model->delete_student($student_id)){
      // retruns true
      $this->response(array(
        "status" => 1,
  ...
  public function index_get(){
    ...
    $students = $this->expert_api_model->get_students();
